 ...
 */
double range_protect(double value, double min_value, double max_value) {
...
double Adc_Normalize(int value, double max_value, double min_value) 
{
    ...
    // 计算归一化值
    normalized_value = ((value - min_value) / (max_value - min_value)) * 100.0f;
    // 使用range_protect函数确保结果在[1.0, 100.0]范围内
    return range_protect(normalized_value, 1.0, 100.0);
}

/**
 * Inductance_Count_Err - 差比和计算
 * 该函数接收四个电感值作为参数，计算电感误差并将其限制在-100到100之间。
 * @param L  电感值L
 * @param LM 电感值LM
 * @param RM 电感值RM
 * @param R  电感值R
...
int16 Inductance_Count_Err(uint16 L, uint16 LM, uint16 RM, uint16 R) 
{
    float scaled_err;
    // 1. 转换为有符号运算避免溢出
    float numerator = Str_value * (float)((int32)L - R) + Cur_value * (float)((int32)LM - RM);
    float denominator = Str_value * (float)(L + R) + Cur_value * (float)(LM + RM);
    // 2. 计算并钳制结果
