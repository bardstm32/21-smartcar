...
// 备注信息     内部调用
//-------------------------------------------------------------------------------------------------------------------
static uint8 soft_spi_8bit_data_handler (soft_spi_info_struct *soft_spi_obj, const uint8 dat)
{
    uint8 temp = 0;
    uint8 write_data = dat;
    uint8 read_data = 0;
...
//    }
    ...
    if(!(soft_spi_obj->config.mode % 2))                                      // CPHA = 0 第一个边沿采样
    {
...
// 备注信息     内部调用
//-------------------------------------------------------------------------------------------------------------------
static uint16 soft_spi_16bit_data_handler (soft_spi_info_struct *soft_spi_obj, const uint16 dat)
{
    uint8 temp = 0;
    uint16 write_data = dat;
    uint16 read_data = 0;
...
void soft_spi_write_8bit (soft_spi_info_struct *soft_spi_obj, const uint8 dat)
{
    ...
    soft_spi_8bit_data_handler(soft_spi_obj, dat);
...
// 备注信息     
//-------------------------------------------------------------------------------------------------------------------
void soft_spi_write_16bit (soft_spi_info_struct *soft_spi_obj, uint16 dat)
{
    zf_assert(soft_spi_obj != NULL);
	if(soft_spi_obj->config.use_cs)
    {
    ...
    soft_spi_16bit_data_handler(soft_spi_obj, dat);
...
// 备注信息     
//-------------------------------------------------------------------------------------------------------------------
void soft_spi_write_16bit_register (soft_spi_info_struct *soft_spi_obj, const uint16 register_name, uint16 dat)
{
    zf_assert(soft_spi_obj != NULL);
	if(soft_spi_obj->config.use_cs)
    {
...
uint8 soft_spi_read_8bit (soft_spi_info_struct *soft_spi_obj)
{
	...
	dat = soft_spi_8bit_data_handler(soft_spi_obj, 0);
...
uint16 soft_spi_read_16bit (soft_spi_info_struct *soft_spi_obj)
{
	...
	dat = soft_spi_16bit_data_handler(soft_spi_obj, 0);
...
void soft_spi_init (soft_spi_info_struct *soft_spi_obj, uint8 mode, uint32 delay, gpio_pin_enum sck_pin, gpio_pin_enum mosi_pin, uint32 miso_pin, uint32 cs_pin)
{
    ...
    else
    {
        ...
        gpio_init(soft_spi_obj->miso_pin, GPI, GPIO_HIGH, GPI_PULL_UP);     // IO 初始化
    }
